 ...
 * 已发布规则引擎
 * @author ly
 * @modifyTime 2020/11/12 9:01:00
 */
@Slf4j
@Service
public class DroolsActionServiceImpl implements DroolsActionService {
    ...
    @Resource
    PublishRuleSceneEntityRelInfoBiz publishRuleSceneEntityRelInfoBiz;
    @Resource
    PublishRuleInfoBiz publishRuleInfoBiz;
    @Resource
    PublishRuleActionInfoBiz publishRuleActionInfoBiz;
    @Resource
    PublishRuleConditionBiz publishRuleConditionBiz;
    @Resource
     ...
     * 获取规则模板
     * @author ly
     * @modifyTime 2020/11/12 9:02:00
     */
    public String getTemplate(String scene,Integer version){
        String droolRuleStr = "";
        PublishRuleSceneInfo sceneInfo = this.publishRuleSceneBiz.getInfoBySecne(scene,version);
        if(sceneInfo!=null){
            if(StringUtils.isNotBlank(sceneInfo.getSceneDrl())){
                log.info("从数据库中读取规则文件");
                droolRuleStr = sceneInfo.getSceneDrl();
            }else{
                log.info("重新编译规则");
                //重新编译规则，然后执行
                droolRuleStr = compileRule(scene,version);
                sceneInfo.setSceneDrl(droolRuleStr);
                this.publishRuleSceneBiz.saveOrUpdate(sceneInfo);
            }
        }
    ...
    public RuleExecutionObject excute(RuleExecutionObject ruleExecutionObject, final String scene,Integer version, String droolRuleStr)  {
//        try {
//            //获取ksession
...
//            } else {
//                //重新编译规则，然后执行
//                droolRuleStr =  getTemplate(scene,version);
//                return compileRuleAndexEcuteRuleEngine(droolRuleStr,ruleExecutionObject, scene,version);
//            }
//        } catch (Exception e) {
//            e.printStackTrace();
...
//        }
    ...
    private RuleExecutionObject compileRuleAndexEcuteRuleEngine(String droolRuleStr, RuleExecutionObject ruleExecutionObject,String scene,Integer version) throws Exception {
        //KieSession对象
        KieSession session;
        try {
            scene = scene+version;
...
//            QueryResults results = session.getQueryResults("");
     ...
     */
    public String compileRule(String scene,Integer version)  {
        //拼接规则脚本
        ...
        //传参
        PublishRuleSceneInfo sceneInfo = new PublishRuleSceneInfo();
        sceneInfo.setSceneIdentify(scene);
        sceneInfo.setPublishVersion(version);
        List<RuleEntityInfo> entityList = this.publishRuleSceneEntityRelInfoBiz.findBaseRuleEntityListByScene(sceneInfo);
        log.info("场景对应的实体个数为:{}", entityList.size());
        // 4.根据场景加载可用的规则信息
        List<PublishRuleInfo> ruleList = this.publishRuleInfoBiz.findBaseRuleListByScene(sceneInfo);
        ...
        for (PublishRuleInfo ruleInfo : ruleList) {
    ...
    private StringBuffer insertImportInfo(StringBuffer droolRuleStr, List<RuleEntityInfo> entityList,
                                          PublishRuleSceneInfo sceneInfo) {
        ...
        // 3.导入动作类
        //根据场景获取动作类信息
        List<PublishRuleActionInfo> actionList = this.publishRuleActionInfoBiz.findRuleActionListByScene(sceneInfo);
        if (StringUtil.listIsNotNull(actionList)) {
            //是否有实现类动作
            Boolean implFlag = false;
            //循环处理
            for (PublishRuleActionInfo actionInfo : actionList) {
...
//                droolRuleStr.append("import").append(" ").append(actionInfo.getActionClass()).append(";").append(lineSeparator);
     ...
     * @param ruleInfo 规则
     */
    private StringBuffer getDroolsInfoByRule(PublishRuleInfo ruleInfo,PublishRuleSceneInfo sceneInfo,List<RuleEntityInfo> entityList)  {
        //拼接规则字符串
        StringBuffer sb = new StringBuffer();
        // 1.拼接规则自身属性信息
        sb = this.insertRuleInfo(sb, ruleInfo);
        // 2.拼接条件
        sb = this.insertRuleCondition(sb, ruleInfo,sceneInfo,entityList);
        // 3.拼接动作
     ...
     * @param ruleInfo 规则
     */
    private StringBuffer insertRuleInfo(StringBuffer ruleStr, PublishRuleInfo ruleInfo)  {
        // 1.拼接规则名称(默认带双引号)
        ruleStr.append(lineSeparator).append("rule").append(" ").append("\"").append(ruleInfo.getRuleName()).append("\"").append(lineSeparator);
        // 2.拼接自身属性
        List<RulePropertyRelInfoVo> rulePropertyList = this.publishRuleInfoBiz.findRulePropertyListByRuleCode(ruleInfo.getRuleCode(),ruleInfo.getPublishVersion());
     ...
     * @param ruleInfo 规则
     */
    private StringBuffer insertRuleCondition(StringBuffer ruleStr, PublishRuleInfo ruleInfo,PublishRuleSceneInfo sceneInfo,List<RuleEntityInfo> entityList)  {
        // 1.拼接when
        ...
        Integer count = this.publishRuleActionInfoBiz.findRuleActionCountByRuleCodeAndActionType(ruleInfo.getRuleCode(),ruleInfo.getPublishVersion());
        if (count > 0) {
            ruleStr.append("$action").append(":").append("DroolsActionService()").append(lineSeparator);
        }

        // 3.根据规则id获取条件信息
        List<PublishRuleConditionInfo> conList = this.publishRuleConditionBiz.findRuleConditionInfoByRuleCode(ruleInfo.getRuleCode(),ruleInfo.getPublishVersion());


        //5.找出动作中的实体在条件中没有的，进行初始化定义
        PublishRuleActionInfo info = new PublishRuleActionInfo();
        info.setRuleCode(ruleInfo.getRuleCode());
        info.setPublishVersion(ruleInfo.getPublishVersion());
        List<PublishRuleActionInfo> actionList = this.publishRuleActionInfoBiz.list(info);

        List<Long> ids = CompareUtil.getPublishConditionId(conList);

        List<PublishRuleActionInfo> addList = CompareUtil.getAddPublishActionList(actionList,conList);
        if(!CollectionUtils.isEmpty(addList)){
            for (PublishRuleActionInfo action : addList) {
    ...
    private StringBuffer insertRuleConditionFromList(StringBuffer ruleStr,List<PublishRuleConditionInfo> conList){
        Map<String,List<PublishRuleConditionInfo>> conditionMap = new HashMap<>();
        for (PublishRuleConditionInfo ruleConditionInfo : conList) {
            List<PublishRuleConditionInfo> list  = conditionMap.get(ruleConditionInfo.getLeftEntityId()+"");
        ...
        for (String key : conditionMap.keySet()) {
            ...
            List<PublishRuleConditionInfo> list = conditionMap.get(key);
     ...
     * @param conList 条件集合
     */
    private StringBuffer insertRuleConditionFromEntity(StringBuffer ruleStr, List<PublishRuleConditionInfo> conList,RuleEntityInfo entityInfo) {
        ...
        for (int c = 0; c < conList.size(); c++) {
            PublishRuleConditionInfo conditionInfo = conList.get(c);
            //表达式
...
//                            expression = expression.replace(conditionVariable, "'" + conditionVariable + "'");
                ...
                ruleStr.append(FiledUtil.getEntityClazz(entityInfo.getPkgName())).append("(")
                    .append(expression).append(")").append(lineSeparator);
            }
        }

        log.info("拼条件后：[{}]",ruleStr);
        return ruleStr;
    }
    ...
    private StringBuffer insertRuleCOnditionByProperty(StringBuffer ruleStr,List<String> list,String expression){
        ...
        for (int i=0;i<list.size();i++) {
            ...
            if(i==0){
                ...
                ruleStr.append(FiledUtil.getEntityClazz(entityInfo.getPkgName())).append("(").append(expression).append(")").append(lineSeparator);
            }
        }
        return ruleStr;
    }

    private StringBuffer insertRuleConditionFromMap(StringBuffer ruleStr,List<PublishRuleConditionInfo> conList,RuleEntityInfo entityInfo){
     ...
     * @param ruleInfo 规则
     */
    private StringBuffer insertRuleActionInfo(StringBuffer ruleStr, PublishRuleInfo ruleInfo)  {
        // 1.拼接then
        ...
        if (!StringUtil.listIsNotNull(actionList)) {
            ...
            // 3.循环处理每一个动作
            for (PublishRuleActionInfo action : actionList) {
